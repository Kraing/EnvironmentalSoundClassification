...
def CreateTrainingSet10(data, label, name='', batch_size=32):
    ...
    # Data gen
    datagen = tf.keras.preprocessing.image.ImageDataGenerator(width_shift_range=0.05,
...
def Data_Augmentation(data_batch, ratio=0.2):
    ...
    for i in range(len(data_batch)):
        ...
        if(p_r[i]<ratio):
            augmented.append(librosa.effects.pitch_shift(np.asarray(data_batch[i]), 22050, pitch_factor[i]))

        if(n_r[i]<ratio):
            augmented.append(data_batch[i] + np.random.uniform(0.001, 0.05, size=len(data_batch[i])))

        if(i_r[i]<ratio):
            augmented.append(np.flip(data_batch[i]))

    return aug



def train(net, max_epochs, training_dataset, validation_dataset, batch_size, aug_rate=0.2, verbose=True):
    ...
    for epoch in range(max_epochs):
        ...
        for x_batch, y_batch in training_dataset:
            ...
            if (batches >= len(training_dataset) / batch_size):
                break
...
def PiczakNet_Reg(input_shape):
    ...
    # Output layer
    model = tf.keras.layers.Dense(50, activation=None, kernel_regularizer=l2(0.0001), name='out')(model)
...
def ESResNet(input_shape):
    ...
    out = tf.keras.layers.Dense(50, activation=None)(X)
